window.onload = function () {
    ...
    gui.add(this, "speed", 0, 2).name("Speed");
    ...
    g.draw = function () {
        ...
        // draw!
        colorPhase += .1 * speed;
        if (colorPhase > Math.PI) {
            colorPhase -= 2 * Math.PI;
        }

        for (var j = 0; j < numTentacles; j++) {
            // finding a color from the rainbow
            var red = parseInt(Math.sin(colorPhase + (j + redPhase * 6) * Math.PI / 3) * 127 + 128);
            var green = parseInt(Math.sin(colorPhase + (j + greenPhase * 6) * Math.PI / 3) * 127 + 128);
            var blue = parseInt(Math.sin(colorPhase + (j + bluePhase * 6) * Math.PI / 3) * 127 + 128);
